...
class EdgeDetector:
    ...
    def save_images(self, edge_frame, overlay_frame):
        """Save both edge detection and panel detection images"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        camera_id = f"cam{self.current_camera}"
        
        # Save edge detection image
        edge_filename = os.path.join(
            self.output_dir, f"edge_{camera_id}_{timestamp}.jpg")
        cv2.imwrite(edge_filename, edge_frame)
        
        # Save panel detection image
        ...
        try:
            while True:
                ...
                if key == ord('q'):
                    ...
                    self.canny_high -= 10
                # Save images on any other key press
                elif key != 255:  # 255 means no key was pressed
                    self.save_images(edge_frame, overlay_frame)
